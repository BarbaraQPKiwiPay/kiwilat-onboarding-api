 ...
 * Provides endpoints for managing quotes associated with loans.
 */
@RestController
@RequestMapping("/api/v1")
@Tag(name = "Quote Controller", description = "Manages quotes for loan applications")
public class QuoteController {
    ...
    @PostMapping("/loans/{loanId}/quotes")
    @Operation(summary = "Create a new quote for a loan", description = "Creates a new quote associated with the specified loan")
    @ApiResponses(value = {
    ...
    public ResponseEntity<QuoteResponse> createQuote(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(quoteCommandService.createQuote(loanId, request));
    }

    @GetMapping("/loans/{loanId}/quotes")
    @Operation(summary = "Get all quotes by loan ID", description = "Retrieves all quotes associated with the specified loan")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Quotes retrieved successfully"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    public ResponseEntity<List<QuoteResponse>> getQuotesByLoanId(@PathVariable Long loanId) {
        return ResponseEntity.ok(quoteQueryService.getQuotesByLoanId(loanId));
    }
    ...
    @GetMapping("/quotes")
    @Operation(summary = "Get all quotes", description = "Retrieves all quotes in the system")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Quotes retrieved successfully"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    public ResponseEntity<List<QuoteResponse>> getAllQuotes() {
        return ResponseEntity.ok(quoteQueryService.getAllQuotes());
    }
}
